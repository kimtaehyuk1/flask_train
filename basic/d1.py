'''
    ...
    6. 회원가입 - > insert
        ...
        - `키워드를 테이블명, 컬럼명등등 사용하면 빽틱을 통해 처리 가능`
        - '값'
    ...
    9. 로그인 -> select(조회)(쿼리의 분량이 가장 많다)

    -- 회원 가입 여부 조회-> 로그인 처리 쿼리
    -- 대소문자 구분 안한다.
    # 제시한 아이디와 비번과 일치하는 row데이터를 가져와서 uid,name,regdate값을 출력하시오.
    SELECT
        uid, `NAME`, regdate
    FROM
        users
...
connection = None # 보험처리
try:
    # 1. pymysql 접속
    connection = my.connect(host='localhost', # 127.0.0.1도 상관없음, 서버주소임
                                #port= 3306,  # 포트
                                user='root', # 사용자 계정, root 계정이 아닌것을 사용 권장 
